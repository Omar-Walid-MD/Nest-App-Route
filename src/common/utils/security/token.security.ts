...
export const createLoginCredentials = async(user: UserDocument) => {
    
    // const signatureLevel = await detectSignatureLevel(user.role);
    // let signatures = await getSignatures(signatureLevel);
    let signatures = {
        access_signature: process.env.ACCESS_USER_TOKEN_SIGNATURE as string,
        refresh_signature: process.env.REFRESH_USER_TOKEN_SIGNATURE as string
    };
